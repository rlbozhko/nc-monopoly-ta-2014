...
public class BuildingDao implements Dao<Building> {
	...
	public List<Building> getAllByParentKey(int key) {
		String sql = "select property_cells.object_id property_cell_id, building.name building_type, "
				...
				+ "  attributes currentPrice, attributes maxLevel, attributes primaryCost " + "where "
				+ "  property_cells.object_id = " + key + " and" + "  building.object_type_id = 5 and"
				+ "  building.parent_id = property_cells.object_id and" + "  currentLevel.attr_id = 24 and"
				+ "  currentLevel.object_id = building.object_id and" + "  currentPrice.attr_id = 25 and"
				+ "  currentPrice.object_id = building.object_id and" + "  maxLevel.attr_id = 27 and"
				+ "  maxLevel.object_id = building.object_id and" + "  primaryCost.attr_id = 29 and"
		...
		List<Building> buildings = this.jdbcTemplate.query(sql, new RowMapper<Building>() {
			public Building mapRow(ResultSet rs, int rowNum) throws SQLException {
				...
				building.setCurrentPrice(Integer.parseInt(rs.getString("CURRENTPRICE")));
